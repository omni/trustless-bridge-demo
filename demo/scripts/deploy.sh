...
CONTRACTS=../contracts
BEACON_LIGHT_CLIENT=$CONTRACTS/light_client/BeaconLightClient.sol:BeaconLightClient
LIGHT_CLIENT_CHAIN=$CONTRACTS/light_client/LightClientChain.sol:LightClientChain
...
echo "Deploying BeaconLightClient pair"

HOME_LIGHT_CLIENT=$(deploy $HOME_RPC_URL $BEACON_LIGHT_CLIENT \
  $FOREIGN_GENESIS_VALIDATORS_ROOT $FOREIGN_GENESIS_TIME $LIGHT_CLIENT_UPDATE_TIMEOUT "($FOREIGN_GENESIS_HEADER_DATA)")
echo "Deployed Home BeaconLightClient at $HOME_LIGHT_CLIENT"

FOREIGN_LIGHT_CLIENT=$(deploy $FOREIGN_RPC_URL $BEACON_LIGHT_CLIENT \
  $HOME_GENESIS_VALIDATORS_ROOT $HOME_GENESIS_TIME $LIGHT_CLIENT_UPDATE_TIMEOUT "($HOME_GENESIS_HEADER_DATA)")
echo "Deployed Foreign BeaconLightClient at $FOREIGN_LIGHT_CLIENT"


echo "Deploying LightClientChain pair"

HOME_LIGHT_CLIENT_CHAIN=$(deploy $HOME_RPC_URL $LIGHT_CLIENT_CHAIN $HOME_LIGHT_CLIENT)
echo "Deployed Home LightClientChain at $HOME_LIGHT_CLIENT_CHAIN"

FOREIGN_LIGHT_CLIENT_CHAIN=$(deploy $FOREIGN_RPC_URL $LIGHT_CLIENT_CHAIN $FOREIGN_LIGHT_CLIENT)
echo "Deployed Foreign LightClientChain at $FOREIGN_LIGHT_CLIENT_CHAIN"
...
HOME_AMB_INIT_DATA=$(cast cd 'initialize(address,uint256,address)' $HOME_LIGHT_CLIENT_CHAIN 2000000 $FOREIGN_AMB)
FOREIGN_AMB_INIT_DATA=$(cast cd 'initialize(address,uint256,address)' $FOREIGN_LIGHT_CLIENT_CHAIN 2000000 $HOME_AMB)
...
HOME_LIGHT_CLIENT_CHAIN=$HOME_LIGHT_CLIENT_CHAIN
HOME_AMB=$HOME_AMB
HOME_AMB_IMPL=$HOME_AMB_I
HOME_OB=$HOME_OB
HOME_OB_IMPL=$HOME_OB_I
HOME_WETH=$HOME_WETH
HOME_ROUTER=$HOME_ROUTER
FOREIGN_LIGHT_CLIENT=$FOREIGN_LIGHT_CLIENT
FOREIGN_LIGHT_CLIENT_CHAIN=$FOREIGN_LIGHT_CLIENT_CHAIN
FOREIGN_AMB=$FOREIGN_AMB
FOREIGN_AMB_IMPL=$FOREIGN_AMB_I
FOREIGN_OB=$FOREIGN_OB
FOREIGN_OB_IMPL=$FOREIGN_OB_I
